---
title: "Colorado First Judicial Recovery Court Semi-Annual Evaluation: 6-Month Report for FY2024-25"
output:
...
From September 29, 2024 through March 31, 2025, the First Judicial
Recovery Court (FJRC) enrolled **32 new participants**, exceeding the
...
age_plot_data <- data_recoded %>%
  ...
  mutate(
    ...
    is_highlight = age_group == age_group[1]  # top group = highlight
  )

# Plot
age_plot <- ggplot(age_plot_data, aes(x = fct_rev(age_group), y = n)) +
  geom_col(aes(fill = is_highlight), width = 0.6) +
  geom_text(
    aes(label = n, color = is_highlight),
    hjust = 2, fontface = "bold"
  ) +
  scale_fill_manual(values = c("TRUE" = "#314160", "FALSE" = "lightgray"), guide = "none") +
  scale_color_manual(values = c("TRUE" = "white", "FALSE" = "black"), guide = "none") +
  coord_flip() +
  labs(
    title = "The majority of participants were 35 to 44 years old",
    x = NULL,
    y = NULL
  ) +
  theme_omni() +
  theme(  axis.text.x = element_blank(),  # removes x-axis numbers
...
relationship_plot <- ggplot(relationship_summary, aes(y = fct_rev(Value), x = count)) +
  geom_col(width = 0.6, fill = "#314160") +             # ← match bar width here
